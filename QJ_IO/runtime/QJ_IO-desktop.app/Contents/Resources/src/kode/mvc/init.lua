...
function kode.facade:notifyObservers(event)
	local observers
    if self.observerMap_[event.name] == nil then
        print("通知事件["..event.name.."]未注册")
    end
...
function kode.facade:send(eventName, ...)
    assert(eventName~=nil,"通知事件的名称不能为空")
...
function kode.facade:registerModules(modules)
	for _, v in ipairs(modules) do
		if v then
			-- v is module name
--			print("module name = "..v)
            Event.register(v, function() return
                g_Event[v]
            end
            )
		end
...
function kode.facade:registerModule(module)
    ...
    if listeners_ == nil then
        print("modules_init_中<"..module..">模块注册事件为空")
        
	end
    listeners_ = table.values(listeners_)
...
function kode.facade:loadController(module, controller)
	...
	--测试使用，正式版本应该删除
	--检查所有事件是否都被注册
        for key,var in pairs(obj_) do
            if type(key) == "string" then
                --action_
                if  string.len(key)>7 then
                    local  str = string.sub(key,1,7)
                    if str == "action_" then
                        local strEvent = string.sub(key,8,-2)
                        local moduleEvents =   g_Event[module]
                        local bFind = false                        
                        for key2,var2 in pairs(moduleEvents) do
                              if var2 == strEvent then
                                  bFind = true
                                  break
                              end                        
                        end
                        assert(bFind == true ,"没有找到事件"..key)
